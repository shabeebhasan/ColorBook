...
{
    return [self floodFillFromPoint:startPoint withColor:newColor andTolerance:tolerance useAntiAlias:YES originalImage:observeImage];
}
...
- (UIImage *) floodFillFromPoint:(CGPoint)startPoint withColor:(UIColor *)newColor andTolerance:(int)tolerance useAntiAlias:(BOOL)antiAlias originalImage:(UIImage *)observeImage
{
    @try
    {
        ...
        */
        ...
        /* get rawData of original Image */
        
        // First get the image into your data buffer
        CGImageRef oImageRef = [observeImage CGImage];
        ...
        unsigned char *oImageData = malloc(oHeight * oWidth * 4);
        NSUInteger oBytesPerPixel = CGImageGetBitsPerPixel(oImageRef) / 8;
        NSUInteger oBytesPerRow = CGImageGetBytesPerRow(oImageRef);
        NSUInteger oBitsPerComponent = CGImageGetBitsPerComponent(oImageRef);
        CGContextRef oContext = CGBitmapContextCreate(oImageData, oWidth, oHeight,
                                                     oBitsPerComponent, oBytesPerRow, oColorSpace,
                                                     kCGImageAlphaPremultipliedLast | kCGBitmapByteOrder32Big);
        CGColorSpaceRelease(oColorSpace);
        
        CGContextDrawImage(oContext, CGRectMake(0, 0, oWidth, oHeight), oImageRef);
        
        CGImageRef newImage = CGBitmapContextCreateImage(oContext);
        ...
        unsigned int oByteIndex = (oBytesPerRow * roundf(startPoint.y)) + roundf(startPoint.x) * oBytesPerPixel;
        
        unsigned int ocolor = getColorCode(oByteIndex, oImageData);
        
        /*//black color dissolve
        if (ocolor == 255) {
            return nil;
        }*/
        
        if(ocolor != 4294967295 & ocolor != 0){
            //release all data
        ...
        */
        ...
        while ([points popFront:&x andY:&y] != INVALID_NODE_CONTENT)
        {
            ...
            oByteIndex = (oBytesPerRow * roundf(y)) + roundf(x) * oBytesPerPixel;
            
            color = getColorCode(oByteIndex, oImageData);
            ...
            while (y < height && compareColor(ocolor, color, tolerance) && ncolor != color)
            {
                ...
                oImageData[oByteIndex + 0] = newRed;
                oImageData[oByteIndex + 1] = newGreen;
                oImageData[oByteIndex + 2] = newBlue;
                oImageData[oByteIndex + 3] = newAlpha;
                
                if(x > 0)
                {
                    byteIndex = (bytesPerRow * roundf(y)) + roundf(x - 1) * bytesPerPixel;
                    
                    oByteIndex = (oBytesPerRow * roundf(y)) + roundf(x - 1) * oBytesPerPixel;
        ...
        while ([antiAliasingPoints popFront:&x andY:&y] != INVALID_NODE_CONTENT)
        {
            ...
            if (!compareColor(ncolor, color, 0))
            {
                ...
                if (antiAlias) {
                    ...
                    oImageData[oByteIndex + 0] = (red1 + red2) / 2;
                    oImageData[oByteIndex + 1] = (green1 + green2) / 2;
                    oImageData[oByteIndex + 2] = (blue1 + blue2) / 2;
                    oImageData[oByteIndex + 3] = (alpha1 + alpha2) / 2;
            ...
            if (y>0)
            {
                ...
                oByteIndex = (oBytesPerRow * roundf(y - 1)) + roundf(x) * oBytesPerPixel;
                color = getColorCode(oByteIndex, oImageData);
